...
2. ARIA roles and labels (roles="button". aria-label="Submit")
...
- Store ALL test data in /test-data/<feature-name>.json - never hardcode in speclfiles
...
- Always assert the EXACT success message for each operation:
    Transfer Funds - "Transfer Complete!"
    Open Account - assert the new account number appears in the confirmation
    Bill Payment - assert payment confirmation with payee name and amount
    Login - assert redirect to /overview and user greeting
